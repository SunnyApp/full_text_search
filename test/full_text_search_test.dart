...
void main() {
  var one = Searchable(name: "John Bob Richards", phone: "480-123-2313", address: "123 East Johnson Lane");
  var two = Searchable(name: "Joe John Johnson", phone: "john@john.com", address: "44 E Johns Creek Eight");
  ...
  test("When isMatchAll = true, natural sorting is retained", () async {
    final List<Searchable> matches = await FullTextSearch(
        term: "John",
        items: searchables,
        isMatchAll: true,
        tokenize: (s) => [s.name, s.phone, s.address]).findResults();
    expect(matches, containsAllInOrder([one, two, three]));
  });

  test("When isMatchAll = false, we sort by numTokens correctly", () async {
    final List<TermSearchResult<Searchable>> matches2 = await FullTextSearch(
        term: "John",
        items: searchables,
        isMatchAll: false,
        isStartsWith: false,
        tokenize: (s) => [s.name, s.phone, s.address]).execute();
    expect(matches2.map((_) => _.result), containsAllInOrder([two, one, three]));
  });
  ...
  test("When isMatchAll = false, applies proper weights for partial matches (2 term)", () async {
    final matches = await FullTextSearch(
        term: "Mac G",
        items: [bigMacGruber, macdonaldDouglas, mac],
        isMatchAll: false,
        isStartsWith: false,
        tokenize: (s) => [s.name]).execute();
    expect(matches[0].result, macdonaldDouglas);
    expect(matches[1].result, bigMacGruber);
    expect(matches[2].result, mac);
  });
